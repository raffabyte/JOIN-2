...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchBtn) searchBtn.addEventListener('click', clearSearchInput);
});
...
function checkEmptyFiltered(tasks) {
  const cols = ['todoColumn', 'inProgressColumn', 'awaitFeedbackColumn', 'doneColumn'];
  const ids = ['toDoDragArea', 'inProgressDragArea', 'awaitingFeedbackDragArea', 'doneDragArea'];
